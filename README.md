...
- **Optional extras**: voice “satellite rising” alerts, pass **WAV recording**, **Cloudlog** frequency sync, **GPS** (NMEA serial) for station position and optional UTC for tracking, local **OscarWatch Logbook** with ADIF export (**Tools → OscarWatch Logbook**), in-app **FT4** modem for duplex satellite digital (**Tools → FT4**; [help](help/ft4.html))
...
- **FT4 modem**: in-app duplex FT4 (**Tools → FT4**) with waterfall, auto-sequence, PTT methods, slot-gated Doppler, echo calibration, and logbook save ([help](help/ft4.html))
