...
public class PassPolarPlotRenderCacheTests
{
    ...
    /// <summary>
    /// FormattedText keyed by colour returns a different instance when the foreground changes,
    /// so N/E/S/W labels cannot stay stuck on the previous theme's brush.
    /// </summary>
    [Fact]
    public void FormattedTextCache_Get_with_color_key_returns_new_text_for_new_foreground()
    {
        var cache = new FormattedTextCache();

        var light = cache.Get("N", 12, Color.Parse("#2d3748"));
        var dark = cache.Get("N", 12, Color.Parse("#c5d0dc"));
